...
export default function Results() {
    ...
    useEffect(() => {
        Axios.get("/admin/data/results", {})
            .then((res) => {
                const url = `${res.data.data[0].result.split("data")[1]}`;
                            ...
                            <div className="flex flex-col w-full items-center justify-center">
                                ...
                                <table className="w-[80%] font-Poppins ">
                                    <thead className="border-2 border-slate-800 p-3 text-center ">
                                        <tr>
                                            <th className="border-2 border-slate-800 p-3 text-center ">
                                                Name
                                            </th>
                                            <th className="border-2 border-slate-800 p-3 text-center ">
                                                Institution
                                    ...
                                    <tbody className="border-2 border-slate-800">
                                        {jsonData.map((row, index) => (
                                            ...
                                            >
                                                <td className="border-2 border-slate-800 p-3 text-center ">
                                                    {row["Name"]}
                                                </td>
                                                <td className="border-2 border-slate-800 p-3 text-center ">
                                                    {row["Institution"]}
